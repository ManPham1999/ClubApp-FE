...
const ClubsDetail = ({
	...
	useEffect(() => {
		...
		Axios({
			method: 'GET',
			url: `https://funnyclub-be.herokuapp.com/api/club/getAllScheduleOfClub/${id}`,
			headers: {
			...
			.then(() => {
				Axios({
					method: 'GET',
					url: `https://funnyclub-be.herokuapp.com/api/club/getAllFeedback/${id}`,
					headers: {
						Accept: 'application/json',
					},
				})
	...
	const onSendReQuest = () => {
		...
		return Axios({
			method: 'POST',
			url: `https://funnyclub-be.herokuapp.com/api/student/joinClub/${id}`,
			data: {requestConfirm},
			headers: {
				Accept: 'application/json',
				Authorization: `${Authorization}`,
	...
	const onSendFeedBack = () => {
		...
		Axios({
			method: 'POST',
			url: `https://funnyclub-be.herokuapp.com/api/club/sendFeedBackOfClub/${id}`,
			data: {feedBack, rate},
			headers: {
				Accept: 'application/json',
				Authorization: `${Authorization}`,
								...
								<Row>
									...
									<Col xs={12} sm={12} lg={4}>
										...
										{userEvents
											.filter((item) => {
												return (
													item.Club._id === club._id && item.isActive === true
												);
											})
											.slice(0, 3)
											.map((event, index) => {
